  ...
  verification: async (req, res) => {
    try {
      ...
      if (req.dataToken.iduser) {
        await dbQuery(`UPDATE user set status=2 WHERE iduser=${dbConf.escape(req.dataToken.iduser)}`)

        //. proses login 
        let resultUser = await dbQuery(`Select u.iduser, u.name, u.email, u.role, u.phone_number, u.gender, u.birthdate, u.profilpict, u.status from user u
                join status s on u.status = s.idstatus WHERE iduser=${dbConf.escape(req.dataToken.iduser)}`)
        if (resultUser.length > 0) {
          // login berhasil
          let token = createToken({ ...resultUser[0] })
          res.status(200).send({
